...
/**
 * REST controller for managing {@link com.brighthr.exam.domain.File}.
 */
@RestController
@RequestMapping("/api")
public class FileResource {
    ...
    /**
     * * @code GET  /files/folder/:id} : get the file by file id
     * @param id
     * @return the {@link List<ResponseEntity> with status {@code 200 (OK)} and with body the fileDTO, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/files/folder/{id}")
    public ResponseEntity<List<FileDTO>> getAllFilesByFolderId(@PathVariable Long id) {
        log.debug("REST request to get File by folder id : {}", id);

        List<FileDTO> fileDTOs = fileService.findAllByFolderId(id);

        return new ResponseEntity<List<FileDTO>>(fileDTOs, HttpStatus.OK);
    }

    @GetMapping("/files/search/{name}")
    public ResponseEntity<List<FileDTO>> getAllFilesByFileName(@PathVariable String name) {
        log.debug("REST request to get File by file name : {}", name);

        List<FileDTO> fileDTOs = fileService.findAllByFileName(name);
