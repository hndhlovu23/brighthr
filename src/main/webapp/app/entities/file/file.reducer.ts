...
export const getAllEntitiesByFileId = createAsyncThunk('file/fetch_all_entities_by_file', async ({ id }: { id: number }) => {
  const requestUrl = `${apiUrl}/folder/${id}`;
  return axios.get<IFile[]>(requestUrl);
});

export const searchAllEntitiesByName = createAsyncThunk('file/search_all_entities_by_name', async ({ name }: { name: string }) => {
